...
function parseStyleDoc(block) {
  ...
  const titleMatch = block.match(/\/\*\* *styleDoc *(-|\d+)? *([^\n]+)/);
  let parsedData = {
    title: '',
    order: isNaN(titleMatch[1]) ? 0 : parseInt(titleMatch[1]),
    description: '',
    example: '',
    css: ''
  };
  ...
  if (titleMatch) {
    parsedData.title = titleMatch[2].trim();
...
function sortNestedTree(treeNode) {
  treeNode.children.sort((a, b) => {
    if (!a.doc || !b.doc) {
      return 0;
    }
    return a.doc.order < b.doc.order ? -1 : a.doc.order > b.doc.order ? 1 : 0; 
  });
...
async function extractStyleDocs(dir) {
  ...
  const sortedDocsTree = sortNestedTree(docsTree);

  return { docsList, docsTree, sortedDocsTree, cssFilePaths: Array.from(cssFilePaths) };
}
